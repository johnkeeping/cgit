...
static void querystring_cb(const char *name, const char *value)
{
	...
	if (!strcmp(name,"r")) {
		...
		ctx.qry.id = xstrdup(value);
		ctx.qry.has_id = 1;
	} else if (!strcmp(name, "id2")) {
		ctx.qry.id2 = xstrdup(value);
		ctx.qry.has_id = 1;
...
static void cgit_parse_args(int argc, const char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (skip_prefix(argv[i], "--cache=", &arg)) {
			...
			ctx.qry.has_symref = 1;
		} else if (skip_prefix(argv[i], "--id=", &arg) ||
			   skip_prefix(argv[i], "--sha1=", &arg)) {
			/* --sha1 is the old name, kept for compatibility */
			ctx.qry.id = xstrdup(arg);
			ctx.qry.has_id = 1;
