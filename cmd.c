...
{
	cgit_print_blob(ctx.qry.id, ctx.qry.path, ctx.qry.head, 0);
}

static void commit_fn(void)
{
	cgit_print_commit(ctx.qry.id, ctx.qry.path);
}

static void diff_fn(void)
{
	cgit_print_diff(ctx.qry.id, ctx.qry.id2, ctx.qry.path, 1, 0);
}

static void rawdiff_fn(void)
{
	cgit_print_diff(ctx.qry.id, ctx.qry.id2, ctx.qry.path, 1, 1);
}
...
{
	cgit_print_log(ctx.qry.id, ctx.qry.ofs, ctx.cfg.max_commit_count,
...
{
	cgit_print_patch(ctx.qry.id, ctx.qry.id2, ctx.qry.path);
}
...
{
	cgit_print_snapshot(ctx.qry.head, ctx.qry.id, ctx.qry.path,
			    ctx.qry.nohead);
}
...
{
	cgit_print_tag(ctx.qry.id);
}

static void tree_fn(void)
{
	cgit_print_tree(ctx.qry.id, ctx.qry.path);
}
